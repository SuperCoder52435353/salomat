...
// Users database persisted in localStorage. Defaults include the requested admin account.
let USERS_DB = {};
...
function loadUsersDB() {
    try {
        ...
        if (saved) {
            USERS_DB = JSON.parse(saved);
        } else {
            // Default users (keys stored lower-case)
            USERS_DB = {
                'user': { password: '1234', role: 'user', name: 'User' },
                'agent': { password: 'PASSWORDABDURAXMON', role: 'admin', name: 'Agent' },
...
class AuthSystem {
    ...
    handleRegister() {
        ...
        if (password.length < 8) {
            this.showNotification('Parol kamida 8 belgidan iborat bo\'lishi kerak.', 'error');
            return;
        }

        if (password !== password2) {
            this.showNotification('Parollar mos emas. Iltimos qayta tekshiring.', 'error');
            return;
        }

        const key = username.toLowerCase();
        if (USERS_DB[key]) {
            this.showNotification('Bu username allaqachon mavjud. Boshqa username tanlang.', 'error');
            return;
        }
        ...
        this.showNotification('Ro\'yxatdan o\'tildi — endi tizimga kirishingiz mumkin.', 'success');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log('Available users:', Object.keys(USERS_DB).join(', '));
    console.log('Admin credentials: agent / PASSWORDABDURAXMON');
});
