...
class MathSolver {
    ...
    setupTextInput() {
        ...
        const exampleSystem = document.getElementById('exampleSystem');
        const exampleGeometry = document.getElementById('exampleGeometry');
        ...
        // Example buttons
        exampleSystem?.addEventListener('click', () => {
            if (textInput) {
                textInput.value = '2x + 3y = 5; 4x - y = 1';
                this.solveFromText();
            }
        });

        exampleGeometry?.addEventListener('click', () => {
            if (textInput) {
                textInput.value = 'triangle 3 4 5';
                this.solveFromText();
            }
        });
    }
    ...
    detectProblemType(problem) {
        // Basic detection with system detection (multiple equations)
        const eqCount = (problem.match(/=/g) || []).length;
        // If multiple equations and variables present, treat as a system
        if (eqCount >= 2 && /[a-zA-Z]/.test(problem)) return 'system';
    ...
    displaySolution(problem, solution) {
        ...
        if (!solutionBox) {
            console.error('❌ solutionBox element not found');
            return;
        }
        
        console.log('displaySolution called with problem:', problem, 'solution:', solution);
        ...
        solutionBox.style.display = 'block';  // Ensure it's visible
...
MathSolver.prototype.solveProblem = async function(problem) {
    ...
    try {
        ...
        // Debug log
        console.log('✓ solveProblem solution:', solution);
...
MathSolver.prototype.detectProblemType = function(problem) {
    ...
    // Detect system of equations (multiple '=' signs and multiple variables)
    const eqCount = (problem.match(/=/g) || []).length;
    if (eqCount >= 2 && (problem.includes('x') && problem.includes('y'))) {
        return 'system';
    }
...
MathSolver.prototype.solveAlgebra = function(problem) {
    ...
    try {
        // If system of equations (multiple '=') delegate to system solver
        const eqCount = (problem.match(/=/g) || []).length;
        if (eqCount >= 2) {
            return this.solveSystem(problem, steps);
        }
...
// Supports simple systems like: "2x + 3y = 5, 4x - y = 1" or separated by ';' or newlines
// ─────────────────────────────────────────────────────────────── 

MathSolver.prototype.solveSystem = function(problem, steps) {
    try {
        // Split possible equations by comma, semicolon, or ' and '
        const parts = problem.split(/[,;]| and |\n/).map(s => s.trim()).filter(s => s.includes('='));
        if (parts.length < 2) throw new Error('Sistem uchun kamida 2 ta tenglama kerak');

        // Only handle 2x2 systems for now
        const eqs = parts.slice(0,2);

        const coeffs = [];
        const constants = [];

        // Helper to extract coefficient for a variable
        const getCoeff = (expr, variable) => {
            // match patterns like +3x, -x, x, 2*x
            const re = new RegExp('([+-]?\\s*\\d*\\.?\\d*)\\s*\\*?\\s*' + variable);
        ...
        eqs.forEach((eq, idx) => {
            ...
            if (sides.length !== 2) throw new Error('Tenglama formati noto\'g\'ri: ' + eq);
            const L = sides[0].trim();
            const R = sides[1].trim();

            // Extract numeric RHS value, try to evaluate if expression
            let rhs = parseFloat(R.replace(/[^0-9+\-\.]/g, ''));
            if (isNaN(rhs)) {
                try { rhs = math.evaluate(R); } catch(e) { throw new Error('O\'ng tomon son emas: ' + R); }
            }

            const a = getCoeff(L, 'x');
            const b = getCoeff(L, 'y');

            coeffs.push([a, b]);
            constants.push(rhs);
        });

        // Steps: show parsed system
        steps.push({ step: 1, description: 'Sistemani parsing qilish', expression: `${coeffs[0][0]}x + ${coeffs[0][1]}y = ${constants[0]} , ${coeffs[1][0]}x + ${coeffs[1][1]}y = ${constants[1]}`, explanation: 'Tenglamalardan koeffitsiyentlar olinadi' });

        // Solve 2x2 using determinant formula
        const a11 = coeffs[0][0], a12 = coeffs[0][1];
        const a21 = coeffs[1][0], a22 = coeffs[1][1];
        const b1 = constants[0], b2 = constants[1];

        const det = a11 * a22 - a12 * a21;
        steps.push({ step: 2, description: 'Determinant hisoblash', expression: `det = ${a11}*${a22} - ${a12}*${a21} = ${det}`, explanation: 'Determinant NODIR bo\'lsa, yagona yechim mavjud emas' });

        if (Math.abs(det) < 1e-12) {
            return { answer: 'Sistem bir yoki cheksiz ko\'p yechimga ega (det=0)', steps: steps, explanation: 'Determinant nolga teng, sistem singular' };
        }
        ...
        steps.push({ step: 3, description: 'Ildizlarni Cramer qoidasi bilan topish', expression: `x = (${b1}*${a22} - ${a12}*${b2})/${det} = ${x}`, explanation: 'Cramer qoidasiga ko\'ra hisoblandi' });
        steps.push({ step: 4, description: 'y ni topish', expression: `y = (${a11}*${b2} - ${b1}*${a21})/${det} = ${y}`, explanation: 'Cramer qoidasiga ko\'ra hisoblandi' });

        // Verification
        const v1 = a11 * x + a12 * y;
        const v2 = a21 * x + a22 * y;
        steps.push({ step: 5, description: 'Tekshirish', expression: `${a11}*${x.toFixed(4)} + ${a12}*${y.toFixed(4)} = ${v1.toFixed(4)}, ${a21}*${x.toFixed(4)} + ${a22}*${y.toFixed(4)} = ${v2.toFixed(4)}`, explanation: `Tekshirish natijalari: ${v1.toFixed(4)} ~= ${b1}, ${v2.toFixed(4)} ~= ${b2}` });

        return { answer: { x: x, y: y }, steps: steps, explanation: `Sistemaning yechimi: x = ${x}, y = ${y}` };

    } catch (err) {
        return { answer: 'Xatolik', steps: steps, explanation: 'Sistemni yechishda xatolik: ' + err.message };
    }
};
...
// ─────────────────────────────────────────────────────────────── 
// GENERAL LINEAR SYSTEM SOLVER (NxN) - Uses math.lusolve if available
// Falls back to a simple Gaussian elimination implementation
// Accepts input as multiple equations separated by ',', ';' or newlines
// Example: "2x+3y=5;4x-y=1" or "x+2y=3,3x+4y=7"
// ─────────────────────────────────────────────────────────────── 

MathSolver.prototype.solveSystem = function(problem) {
    const steps = [];
    try {
        // Split into individual equations
        const parts = problem.split(/[,;\n]+/).map(s => s.trim()).filter(s => s.length && s.includes('='));
        if (parts.length < 2) throw new Error('Sistem uchun kamida 2 ta tenglama kerak');

        // Collect variable names (letters) in order of appearance
        const varSet = new Set();
        parts.forEach(p => {
            const vars = p.match(/[a-zA-Z]\w*/g) || [];
            vars.forEach(v => { if (!/^[0-9]+$/.test(v)) varSet.add(v.replace(/[^a-zA-Z]/g, '')); });
        });

        const variables = Array.from(varSet);
        if (variables.length === 0) throw new Error('Sistemda o‘zgaruvchi topilmadi');

        // Build coefficient matrix A and RHS vector b
        const A = [];
        const b = [];

        const parseCoeff = (expr, variable) => {
            // Normalize: replace - with +- for easier splitting
            // Match coefficient before variable (handles x, -x, 2x, 2*x, 0.5x)
            const re = new RegExp('([+-]?\\s*\\d*\\.?\\d*)\\s*\\*?\\s*' + variable + '(?![a-zA-Z0-9_])', 'i');
            const m = expr.match(re);
            if (!m) return 0;
            let s = m[1].replace(/\s+/g, '');
            if (s === '' || s === '+') return 1;
        ...
        parts.forEach((eq) => {
            const sides = eq.split('=');
            const L = sides[0].trim();
            const R = sides.slice(1).join('=').trim();

            // Try evaluate RHS if expression
            let rhs = parseFloat(R.replace(/[^0-9+\-\.eE]/g, ''));
            if (isNaN(rhs)) {
                try { rhs = math.evaluate(R); } catch (e) { throw new Error('O\'ng tomonni o\'qib bo\'lmadi: ' + R); }
            }

            const row = variables.map(v => parseCoeff(L, v));
            A.push(row);
            b.push(rhs);
        });

        steps.push({ step: 1, description: 'Parsing', expression: `Variables: ${variables.join(', ')}`, explanation: 'O‘zgaruvchilar aniqlandi va matritsa qurildi' });
        steps.push({ step: 2, description: 'Matritsa A va vektor b', expression: `A = ${JSON.stringify(A)}, b = ${JSON.stringify(b)}`, explanation: 'Koeffitsiyentlar va RHS tayyor' });

        // Try to use math.lusolve or math.solve if available
        let sol = null;
        try {
            if (typeof math !== 'undefined' && typeof math.lusolve === 'function') {
                // math.lusolve expects matrix and column vector
                const res = math.lusolve(A, b);
                // res may be nested arrays
                sol = (Array.isArray(res) && Array.isArray(res[0])) ? res.map(r => r[0]) : res;
            } else if (typeof math !== 'undefined' && typeof math.solve === 'function') {
                // Fallback: try solve for vector symbolically if available
                // Build symbol vector and use math.solve if API supports
                // Not all math.js builds support this; detect gracefully
                sol = null;
            }
        } catch (e) {
            sol = null;
        ...
        // If no math solver available, implement simple Gaussian elimination
        if (!sol) {
            const gaussianSolve = (AA, bb) => {
                const n = AA.length;
                // Deep copy
                const M = AA.map(r => r.slice());
                const B = bb.slice();
                ...
                for (let k = 0; k < n; k++) {
                    ...
                    if (Math.abs(M[i_max][k]) < 1e-12) throw new Error('Sistem singular yoki yechim topib bo\'lmaydi');
                    // Swap rows
            ...
            sol = gaussianSolve(A, b);
        }

        // Build result mapping
        const result = {};
        variables.forEach((v, i) => { result[v] = parseFloat((sol[i] || 0).toFixed(8)); });

        // Steps: show solution
        steps.push({ step: 3, description: 'Yechim', expression: JSON.stringify(result), explanation: 'Sistemning yechimi hisoblandi' });

        // Verification
        const verify = A.map((row, i) => row.reduce((s, coeff, j) => s + coeff * result[variables[j]], 0));
        steps.push({ step: 4, description: 'Tekshirish (A*x)', expression: `Ax = ${JSON.stringify(verify)}`, explanation: `Tekshirish: natija RHS ga mos kelishi kerak (${JSON.stringify(b)})` });

        return { answer: result, steps: steps, explanation: `Sistemaning yechimi: ${JSON.stringify(result)}` };
