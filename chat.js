...
class ChatSystem {
    ...
    init() {
        ...
        window.addEventListener('storage', (event) => {
            if (event.key === 'chatConversations') {
                console.log('💬 Chat data synced from another tab');
                this.loadConversations();
                this.checkForUpdates();
            }
        });
        
        // Start periodic sync (backup for single-tab usage)
        this.startSync();
    ...
    loadConversations() {
        try {
            // Use localStorage for cross-device sync (storage events work with localStorage)
            ...
            console.log('✓ Conversations loaded:', Object.keys(this.conversations).length, 'users');
        } catch (error) {
    ...
    sendUserMessage() {
        ...
        // Save to sessionStorage (syncs across tabs/devices on this browser)
        this.saveConversations();
        
        // Render user's chat modal
        this.renderUserMessages();
        
        // Update admin's chat list (if admin has tab open)
    ...
    sendAdminMessage(userId) {
        ...
        // Save to sessionStorage (this syncs to all tabs and devices on this browser)
        this.saveConversations();
        
        // Mark user's messages as read (user will see admin reply)
        this.markAsRead(userId, 'user');
        ...
        // Log for debugging
        console.log('✓ Admin message sent to:', userId, 'Message:', message);
    }
    ...
    checkForUpdates() {
        // Reload conversations from sessionStorage to sync across tabs/devices
        this.loadConversations();
        
        // Update badges to show unread message counts
        this.updateBadges();
        
        const currentUser = window.authSystem?.getCurrentUser();
        
        // If admin, update the chat list to show new messages
        if (currentUser?.role === 'admin' && this.currentConversation) {
            this.renderAdminMessages(this.currentConversation);
            this.updateAdminChatList();
        }
        
        // If regular user, check for new admin messages
        if (currentUser?.role === 'user') {
            const conversation = this.conversations[currentUser.username];
            if (conversation && document.getElementById('userChatMessages')) {
                this.renderUserMessages();
            }
        }
    }
